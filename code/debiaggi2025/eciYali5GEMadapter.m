classdef eciYali5GEMadapter < ModelAdapter
    methods
        function obj = eciYali5GEMadapter()
            % Set initial values of the obj.params - they can be changed by the user
            
             % Automatically find the folder containing this adapter
            adapterFolder = fileparts(mfilename('fullpath'));

            % Move two levels up: from /code/debiaggi2025/ to /eciYali5-GEM/
            repoRoot = fileparts(fileparts(adapterFolder));

            % Path to the folder where all model-specific files and scripts are kept
            obj.params.path = repoRoot;

            % Path to the conventional GEM that this ecModel will be based on
            obj.params.convGEM = fullfile(repoRoot, 'model', 'iYali5-GEM', 'iYali5.xml');
