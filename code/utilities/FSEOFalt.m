function fseof = FSEOFalt(model,modelType,prodTargetRxn,csRxn,nSteps,filterComplexRxns,outputFile,outputFileID,filePath,modelAdapter)
% FSEOFalt
%   Function that runs Flux-Scanning with Enforced Objective Function (FSEOF)
%   for a specified production target.
%
% Input:
%   model          a model structure, or an ecModel in GECKO 3 format (with ecModel.ec structure).
%   modelType      to specify if it is a regular GEM (default) or ecGEM
...
%                  default false)
%   outputFileID   Unique identifier in constructing the names of the output files where the results of the FSEOF analysis are saved
...
%   filterComplexRxns   Boolean to specify if filtering out reactions
%                       involved in complexes should be performed.
...
% Validate modelType
validModelTypes = {'GEM', 'ecGEM'};
if ~ismember(modelType, validModelTypes)
    error('Invalid modelType. Expected ''GEM'' or ''ecGEM'', but got: %s', modelType);
end

if ~strcmp(modelType, 'ecGEM')
    modelType = 'GEM';
end



if nargin < 10 || isempty(modelAdapter)
...
if nargin < 9 || isempty(filePath)
    filePath = fullfile(params.path, 'output');
end

if nargin < 8 || isempty(outputFileID)
    outputFileID = 'ecFSEOF_results';
end
...
if nargin < 6 || isempty(filterComplexRxns)
    filterComplexRxns = false;
end
...
% Filter out reactions involved in complexes
if filterComplexRxns
    complex_rxns = contains(model.grRules(getIndexes(model,target_rxns,'rxns')), 'and');
    target_rxns  = target_rxns(~complex_rxns);
    v_matrix     = v_matrix(~complex_rxns, :);
    rxnGeneM     = rxnGeneM(~complex_rxns, :);
end
...
% Filter out protein usages (they interfere in complex proteins filtering)
toKeep = ~startsWith(target_rxns,'usage_prot_');
target_rxns     = target_rxns(toKeep);
v_matrix        = v_matrix(toKeep,:);
rxnGeneM        = rxnGeneM(toKeep,:);
slope_rxns      = slope_rxns(toKeep);
target_type     = target_type(toKeep);
...
if strcmp(modelType, 'GEM')
    [genesIndex, ~, originalGenes, ~, grRatioMuts]=findGeneDeletions(model,'sgd');
    grRatio=zeros(1,numel(model.genes));
    grRatio(genesIndex) = grRatioMuts;
end
...
for i = 1:numel(genes)
    ...
    if strcmp(modelType,'ecGEM')
        ...
        fetchGeneIdx = strcmpi(originalGenes, genes{i});
        if grRatio(fetchGeneIdx) == 0 || grRatio(fetchGeneIdx) < 1e-8
            essentiality(i) = {'essential'};
        end
    end
...
% Exclude enzyme usage reactions (might be redundant with previous removal)
toKeep = ~startsWith(target_rxns,'usage_prot_');
...
% Initialize rxnNames for each gene
geneRxnNames = cell(size(genes));
geneRxnIds = cell(size(genes));

% Iterate through each gene index to extract associated reaction names
for i = 1:numel(geneIdx)
    % Find reactions connected to the current gene
    rxns_for_gene = find(model.rxnGeneMat(:,geneIdx(i)) > 0);

    % Filter out reactions with IDs starting with 'usage_prot_' and the
    % reverse reactions
    valid_rxns = rxns_for_gene(~startsWith(model.rxns(rxns_for_gene), 'usage_prot_') & ~contains(model.rxns(rxns_for_gene), '_REV'));

    % Remove duplicates from reaction names and reaction IDs
    unique_rxnNames = unique(model.rxnNames(valid_rxns), 'stable'); % 'stable' preserves the order

    % Fetch the reaction names and concatenate them
    geneRxnNames{i} = strjoin(unique_rxnNames, '; ');
    geneRxnIds{i} = strjoin(model.rxns(valid_rxns), '; ');
end
...
fseof.geneTargets(:,6) = geneRxnNames; % Add reaction names to the output
fseof.geneTargets(:,7) = geneRxnIds; % Add reaction names to the output

% Save results in a file if defined
if outputFile
    % Define file prefix based on modelType
    if strcmp(modelType, 'GEM')
        filePrefix = 'FSEOF_';
    else
        filePrefix = 'ecFSEOF_';
    ...
    % Construct file names with fileNameID
    geneFileName = sprintf('%s%s_genes.tsv', filePrefix, outputFileID);
    rxnFileName = sprintf('%s%s_rxns.tsv', filePrefix, outputFileID);
    transportFileName = sprintf('%s%s_transporter.tsv', filePrefix, outputFileID);

    % Write file with gene targets
    writetable(cell2table(fseof.geneTargets, ...
        'VariableNames', {'gene_IDs', 'gene_names', 'slope', 'action', 'essentiality', 'rxnNames', 'rxnIDs'}), ...
        fullfile(filePath, geneFileName), ...
        'FileType', 'text', ...
        'Delimiter', '\t', ...
        'QuoteStrings', false);

    % Write file with reaction targets
    writetable(cell2table(fseof.rxnTargets, ...
        'VariableNames', {'rxn_IDs', 'rxnNames', 'slope', 'grRules', 'rxn_formula'}), ...
        fullfile(filePath, rxnFileName), ...
        'FileType', 'text', ...
        'Delimiter', '\t', ...
        'QuoteStrings', false);

    % Write file with transport reaction targets
    writetable(cell2table(fseof.transportTargets, ...
        'VariableNames', {'rxn_IDs', 'rxnNames', 'slope', 'grRules', 'rxn_formula'}), ...
        fullfile(filePath, transportFileName), ...
        'FileType', 'text', ...
        'Delimiter', '\t', ...
        'QuoteStrings', false);

    disp(['FSEOF results stored at: ' newline filePath]);
end
end